...
def run():
    errors = []
    lx = Lexer(errors=errors, debug=False)
    lx.build()
    lx.lex.input(SOURCE)

    print("TOKENS (keywords, IDs, INDENT/DEDENT):")
    depth = 0
    while True:
        tok = lx.lex.token()
        if not tok:
        ...
        if tok.type == "INDENT":
            ...
            depth -= 1
        print(f"L{tok.lineno:>3}  {tok.type:10} {repr(tok.value):>12}  depth={depth}")

    if errors:
        print("\nLEXICAL ERRORS (first 60 shown):")
        shown = 0
        for e in errors:
            if shown >= 60:
                break
